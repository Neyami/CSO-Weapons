...
const float M950_TIME_DEPLOY		= 1.0f;
...
const string M950_SOUND_EMPTY		= "custom_weapons/cso/dryfire_pistol.wav";
...
class weapon_m950 : ScriptBasePlayerWeaponEntity
{
	...
	bool GetItemInfo( ItemInfo& out info )
	{
		...
		info.iSlot 		= M950_SLOT-1;
		info.iPosition 	= M950_POSITION-1;
		info.iFlags 	= 0;
	...
	bool Deploy()
	{
		bool bResult;
		{
			...
			self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextTertiaryAttack = g_Engine.time + M950_TIME_DEPLOY;
			return bResult;
		}
	}
	...
	void Shoot( bool IsPrimaryAttack )
	{
		...
		GetDefaultShellInfo( EHandle(m_pPlayer), vecShellOrigin, vecShellVelocity, 15, 6, -5 );
		...
		DoDecalGunshot( vecSrc, vecAiming, vecShootCone.x, vecShootCone.y, BULLET_PLAYER_MP5, EHandle(m_pPlayer) );
	...
	void ClipCasting( Vector origin )
	{
		...
		NetworkMessage m950clip( MSG_BROADCAST, NetworkMessages::SVC_TEMPENTITY );
