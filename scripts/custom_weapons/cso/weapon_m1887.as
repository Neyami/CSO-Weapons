...
const string CSOW_NAME								= "weapon_m1887";
...
const float CSOW_DAMAGE								= (80/CSOW_PELLETCOUNT);
const float CSOW_TIME_DELAY						= 0.805;
const float CSOW_TIME_DRAW						= 1.0;
const float CSOW_TIME_IDLE							= 1.0;
const Vector CSOW_OFFSETS_SHELL				= Vector( 8.048010, 4.124480, -1.574783 ); //forward, right, up
const Vector CSOW_VECTOR_SPREAD				= Vector( 0.0675, 0.0675, 0 );

const string CSOW_ANIMEXT							= "shotgun";

const string MODEL_VIEW								= "models/custom_weapons/cso/v_m1887.mdl";
const string MODEL_PLAYER							= "models/custom_weapons/cso/p_m1887.mdl";
const string MODEL_WORLD							= "models/custom_weapons/cso/w_m1887.mdl";
...
const array<string> pCSOWSounds =
{
	...
	"custom_weapons/cso/m1887-1.wav",
	"custom_weapons/cso/m1887_draw.wav",
	"custom_weapons/cso/m1887_insert.wav",
	"custom_weapons/cso/m1887_reafter.wav",
	"custom_weapons/cso/m1887_restart.wav"
};

class weapon_m1887 : CBaseCSOWeapon
{
	...
	void Precache()
	{
		...
		g_Game.PrecacheGeneric( "sprites/custom_weapons/cso/640hud34.spr" );
	}
	...
	bool GetItemInfo( ItemInfo& out info )
	{
		...
		info.iSlot				= cso::M1887_SLOT - 1;
		info.iPosition		= cso::M1887_POSITION - 1;
		info.iWeight			= cso::M1887_WEIGHT;
	...
	void PrimaryAttack()
	{
		...
		if( m_pPlayer.pev.FlagBitSet(FL_ONGROUND) )
			m_pPlayer.pev.punchangle.x -= g_PlayerFuncs.SharedRandomFloat( m_pPlayer.random_seed + 1, 4.72, 7.08 );
		else
			m_pPlayer.pev.punchangle.x -= g_PlayerFuncs.SharedRandomFloat( m_pPlayer.random_seed + 1, 9.44, 12.98 );

		m_flEjectBrass = g_Engine.time + 0.35;
	}
...
void Register()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "cso_m1887::weapon_m1887", CSOW_NAME );
	g_ItemRegistry.RegisterWeapon( CSOW_NAME, "custom_weapons/cso", "buckshot", "", "ammo_buckshot" );
}

} //namespace cso_m1887 END