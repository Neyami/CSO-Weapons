...
const array<string> pCSOWSounds =
{
	...
	"custom_weapons/cs16/knife_slash1.wav",
	"custom_weapons/cso/tomahawk_wall.wav" //"custom_weapons/cs16/knife_hitwall1.wav"
};
//g_MegatronMode = Mode B idle, g_FallenGalvatron = Mode B attacking
enum t9states_e
{
	STATE_MODEA			= 1,
	STATE_CHANGING		= 2,
...
class weapon_thanatos9 : CBaseCustomWeapon //ScriptBasePlayerWeaponEntity
{
	...
	private float m_flPlayHitSound;
	...
	void SecondaryAttack()
	{
		...
		if( (m_iWeaponState & STATE_MODEB) == 0 )
		{
			...
			pev.nextthink = g_Engine.time + 1.0f; //0.75f;
		...
		else
		{
			...
			m_uiSmokeState = 0;
			SetThink( ThinkFunction(this.Create_Smoke) );
			pev.nextthink = g_Engine.time + 2.0f; //0.75f;
	...
	bool ModeBAttack( bool bFirst )
	{
		...
		if( tr.flFraction >= 1.0f )
		{
			if( bFirst )
			{
				...
				g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, pCSOWSounds[SND_KNIFE_SLASH], 1, ATTN_NORM, 0, 94 + Math.RandomLong(0, 0xF) );
		...
		else
		{
			...
			if( m_flPlayHitSound == 0 )
				m_flPlayHitSound = g_Engine.time + Math.RandomFloat(0.025f, 0.03f);
			...
			if( pEntity !is null )
			{
				...
				{
					g_SoundSystem.EmitSound( m_pPlayer.edict(), CHAN_WEAPON, pCSOWSounds[SND_HIT], 1, ATTN_NORM );
			...
			if( bHitWorld )
			{
				...
				if( m_flPlayHitSound > 0 and g_Engine.time > m_flPlayHitSound )
				{
					m_flPlayHitSound = 0;
					g_Game.AlertMessage( at_console, "Play Hit Sound!\n" );
					g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_ITEM, pCSOWSounds[SND_KNIFE_HIT_WALL], VOL_NORM, ATTN_NORM, 0, 98 + Math.RandomLong(0, 3) );
				}
			}
	...
	void Change_Thanatos9()
	{
		...
		if( (m_iWeaponState & STATE_MODEB) == 0 )
		{
			...
			m_iWeaponState = STATE_MODEB; //todo |= ?
...
/*
Todo
weaponstate gets set to 0 if switching weapons during mode B attack
Fix W model so it doesn't sink into the floor
Remove muzzleflash from P model
*/